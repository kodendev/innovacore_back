...
export class MenuController {
    ...
    update(@Param('id') id: number, @Body() updateMenuDto: Partial<CreateMenuDto>) {
    ...
    findWithProducts(@Param('id') id: number) {
        return this.menuService.findWithProducts(Number(id))
    }

    @Get('all/with-products')
    @ApiOperation({ summary: 'Obtener todos los menús con sus productos asociados' })
    @ApiResponse({
        status: 200,
